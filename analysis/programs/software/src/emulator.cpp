...
double emulator::Emulate_NR(Eigen::MatrixXd testX, Eigen::MatrixXd Y, Eigen::MatrixXd &outMatrix){
  unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
  std::default_random_engine generator(seed);
  std::normal_distribution<double> dist(0.0,1.0);
...
void emulator::Emulate_NoPrior(Eigen::MatrixXd testX, Eigen::MatrixXd &outMatrix){
  unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
  std::default_random_engine generator(seed);
  std::normal_distribution<double> dist(0.0,1.0);
  ...
  //matrices and vectors for computational use
  Eigen::MatrixXd KernelSS = Eigen::MatrixXd::Zero(testPoints,testPoints),
  ...
  for(int ob=0;ob<obsCount;ob++){
    ...
    postFunc = L*randomSample;
    
    outMatrix.col(ob*2) = postFunc.col(0);
    for(int tp=0;tp<testPoints;tp++){
      outMatrix(tp,ob*2 + 1) = L(tp,tp);
    }
